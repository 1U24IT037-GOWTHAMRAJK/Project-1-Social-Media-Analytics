...
def parse_label(label):
    ...
    result["state"] = label[state_start:state_end].strip()  
    return result
...
def find_hashtags(message):
    ...
    for i in range(len(message)):
        if message[i] == "#":
            ...
            while end < len(message) and message[end] not in end_chars:
                end+=1
            s = message[start:end]
            hastag.append(s)        
    return hastag        
...
def add_columns(data, state_df):
    ...
    for lable in data["label"]:
        lable_result = parse_label(lable)
        names.append(lable_result["name"])
        positions.append(lable_result["position"])
        state = lable_result["state"]
        states.append(state)
